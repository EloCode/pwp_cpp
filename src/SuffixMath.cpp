...
auto SuffixMath::powContainsPows(const Integer& b, const Integer& n)
    -> const Integer {
...
auto SuffixMath::powContains(const vector<int>& digits,
                             const Integer& b,
                             const Integer& n) -> const Integer {
  array<bool, 10> forbidden = {false, false, false, false, false,
...
auto SuffixMath::powContains(const array<bool, 10>& forbidden,
                             const Integer& b,
                             const Integer& n) -> const Integer {
  ...
  // cout << i << " ";
  // cout << "], b = " << b << ", n = " << n << ")" << endl;
  Integer max_num_digits =
      1 + n * 30103 / 100000;  // 30103 / 100000 >~ log_10(2)
  ...
  while (true) {
    ...
    for (unsigned long m = m_start; m < m_stop; m++) {
      ...
      if (max_num_digits_fits_ui and
          m >= max_num_digits.get_ui()) {  // Checked all digits from b^n
        mpz_clears(suffix, pow10m, digit, NULL);
    ...
    // cout << endl;
    if (m_stop > std::numeric_limits<unsigned long>::max() /
                     10) {  // b^n has more digits but we cannot check since
